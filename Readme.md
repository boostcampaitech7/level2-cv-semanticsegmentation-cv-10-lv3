...
- 손가락 끝 부분 (f1, f4, f8, ,f12, f16) 성능이 낮음 (Val acc **0.95**)
...
위의 2개 모델 간 성능 차이는 약 0.2%로 차이가 있었으며, 학습 시간 역시 약 **40분 차이**에 불과하여 **Base 모델**로 UNet++을 선정하고 다양한 실험을 진행했다. 반면, 다른 모델(Segformer, Convnext, DeepLab3v+)들은 학습 시간이 **6~7시간** 정도 소요되어 Base 모델에서 제외하였다.
